...
def bash_enroll(username):
    os.system('bash enroll.sh {}'.format(username))

def user_enroll(username):
    '''
    根据注册用户名获取注册语音
    ...
    '''
    try:
        enroll_generate(username)
        os.system('bash enroll.sh {}'.format(username))
        # t = threading.Thread(target=bash_enroll, args=(username,))
        # t.start()
        # t.join()
        return "Yes"
    except:
